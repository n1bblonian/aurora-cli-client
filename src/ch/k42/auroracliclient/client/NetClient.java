...
//                        8 quorgs.add(Wave.class);
                        ...
                        switch (classname){
                            case "MailQuorg": //mail quorg
                                ...
                                MailQuorgProperties mprop = new MailQuorgProperties();
                                mprop.setUsername(user)
                                        .setPassword(password);
                                settings = mprop.getSettings();
                                break;
                            case "ColorQuorg": //COLOR quorg
                                ColorQuorgProperties cprop = new ColorQuorgProperties();
                                cprop.setColor(Color.white);
                                settings = cprop.getSettings();
                                break;
                            case "RainbowEffect":
                                l("Raining? y/n");
                                RainbowEffectProperties rprop = new RainbowEffectProperties();
                                if(in.readLine().equals("y")){
                                    rprop.setRaining(true);
                                }else {
                                    rprop.setRaining(false);
                                }
                                settings = rprop.getSettings();
                                break;
                            case "TextEffect": //Text
                                l("Text?");
                                TextEffectProperties tprop = new TextEffectProperties();
                                tprop.setText(in.readLine())
                                        .setForegroundColor(Color.CYAN)
                                        .setBackgroundColor(Color.black)
                                        .setSpeed(1);
                                settings = tprop.getSettings();
                                break;
                        }
                        l("Choose Matrix:");
                        int matrix = -1;
